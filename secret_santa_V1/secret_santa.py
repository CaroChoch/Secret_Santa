...
call with the --send argument:
    ...
    $ python secret_santa.py --send
        ...
        for pair in pairs:
            ...
            # Utilisez MIMEMultipart pour gérer les parties multiples du message (texte, image, etc.)
            msg = MIMEMultipart()
            msg['Date'] = date
            ...
            # Attachez le texte du message
            msg.attach(MIMEText(body, 'plain', 'utf-8'))

            # Obtenez le chemin de l'image depuis le fichier YAML
            image_path = config.get('IMAGE_PATH')

            if image_path:
                # Combinez le chemin actuel avec le chemin relatif de l'image
                image_path = os.path.join(os.path.dirname(__file__), image_path)

                if os.path.exists(image_path):
                    with open(image_path, 'rb') as img_file:
                        image_data = MIMEImage(img_file.read(), name=os.path.basename(image_path))

                        # Attachez l'image à votre message
            ...
            if send:
                # Utilisez msg.as_string() pour envoyer le message avec les pièces jointes (le cas échéant)
